    ...
    if request.method == "POST":
        if "decline" in request.POST or "approve" in request.POST:
            ...
            def send_mail_refund_approved(request, receiver):
                ...
                text = """
                    Xin chao {guestName},
                    Chung toi xac nhan rang yeu cau hoan tien cua ban da duoc chap nhan.
                    So tien hoan tra se duoc chuyen khoan trong vong 24h.
            ...
            def send_mail_refund_declined(request, receiver):
                ...
                text = """
                    Xin chao {guestName},
                    Chung toi rat tiec phai thong bao rang yeu cau hoan tien cua ban da bi tu choi.
                    Sau khi kiem tra chung toi phat hien rang ban da vui pham 1 so chinh sach cua chung toi.*args
                    Chung toi xin loi vi su bat tien nay. Hen gap lai lan sau.
                """
                email_text = text.format(
                    guestName=receiver.user.first_name + " " + receiver.user.last_name)
